...
AWS对MCP身份验证的贡献基于成熟的OAuth标准，实现了自动发现、动态注册和安全保护机制。
...
#### 1. 服务器发现
添加新的MCP服务器从一个URL开始——这个网址告诉MCP客户端在哪里找到服务器。用户通常从已知位置发现这些URL，例如：
- 产品文档
- 在线注册表

#### 2. 元数据收集
作为初始化步骤，MCP客户端需要收集关于如何使用和验证服务器的基本信息：

##### OAuth保护资源元数据
客户端首先请求服务器的OAuth保护资源元数据（技术标准：RFC 9728）。这个元数据就像服务器的名片，包含：
- 如何建立安全连接的关键信息
- OAuth授权服务器的位置
- 访问凭据的可信颁发机构

##### 授权服务器元数据
找到授权服务器后，MCP客户端查询授权服务器的元数据文档（RFC 8414定义的功能）。这告诉客户端：
- 如何与特定授权服务器交互
- 完成安全握手的指令手册

#### 3. 动态客户端注册
有了这些信息，MCP客户端可以通过动态客户端注册向授权服务器介绍自己：

**传统方式 vs MCP方式**
- **传统方式**：管理员手动配置，在系统间复制粘贴凭据
- **MCP方式**：客户端自动提供详细信息并接收唯一标识符

MCP规范使这个过程对最终用户完全透明。

#### 4. OAuth授权流程
此时，MCP客户端已收集了安全连接到特定MCP服务器所需的所有信息。最后一步遵循标准OAuth授权流程：

1. 用户被引导到浏览器中的服务登录页面
2. 服务提供商通过标准OAuth实现处理用户身份验证和权限管理
3. 控制MCP客户端可以访问的数据类型
4. 用户验证并批准权限后，MCP客户端接收安全调用MCP服务器所需的OAuth访问令牌

### 安全考虑

#### 核心权衡
简化合法服务器连接的同时，也使连接到恶意服务器变得更容易。

#### 技术保护措施

##### 凭据防盗保护（RFC 8707）
- 确保为一个服务器颁发的凭据不能被另一个服务器滥用
- 即使用户意外连接到欺骗性服务器（如Examp1e.com而非Example.com），该服务器也无法访问用户在其他地方的合法账户
- MCP客户端将特定服务器URL作为身份验证过程的一部分
- 授权服务器确保凭据严格限制在该服务器

##### 可信注册表
MCP注册表工作组正在开发可信MCP服务器注册表的方法，类似于：
- 应用商店
- 软件包存储库

**目标**：帮助用户通过维护良好的已知MCP实现目录来发现和验证可信服务。

### 技术优势

通过结合以下技术，MCP实现了传统手动设置的自动化，同时为更动态的环境添加了保护措施：
