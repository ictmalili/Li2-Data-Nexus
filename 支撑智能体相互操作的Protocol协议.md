...
![权限认证流程图](https://d2908q01vomqb2.cloudfront.net/ca3512f4dfa95a03169c5a670a4c91a19b3077b4/2025/06/26/sequence-diagram.png)
让我们通过实际案例来了解MCP的工作原理，从用户首次接触MCP服务器开始。

添加新的MCP服务器从一个URL开始——这个网址告诉MCP客户端在哪里找到服务器。用户通常从已知位置发现这些URL，例如产品文档或在线注册表。接下来发生的是一系列步骤，将这个URL转换为安全的、经过身份验证的连接。

作为初始化步骤，该过程从MCP客户端收集关于如何使用和验证服务器的基本信息开始。为了实现这一点，客户端首先请求服务器的OAuth保护资源元数据（对于那些对技术细节感兴趣的人，这在最新标准RFC 9728中定义）。这个元数据就像服务器的名片——它包含关于如何建立安全连接的关键信息，包括在哪里找到OAuth授权服务器，该服务器充当颁发和管理访问凭据的可信权威。

一旦找到授权服务器，MCP客户端会采取另一个准备步骤：它查询授权服务器自己的元数据文档（RFC 8414中定义的功能）。这告诉客户端如何与这个特定的授权服务器交互——类似于获得完成安全握手的指令手册。

有了这些信息，MCP客户端可以通过称为动态客户端注册的过程向授权服务器介绍自己。在这一步中，客户端自动提供关于自己的详细信息并接收回一个唯一标识符。这种自动化方法简化了传统上需要管理员手动配置的过程——在系统之间复制和粘贴凭据。MCP规范使这个过程对最终用户来说是无缝的。

到这一点，MCP客户端已经收集了安全连接到这个特定MCP服务器所需要知道的一切。最后一步遵循标准的OAuth授权流程，用户被引导到浏览器中服务的登录页面。像任何受OAuth保护的服务一样，服务提供商通过其标准OAuth实现处理用户身份验证和权限管理——例如，控制MCP客户端将被允许访问什么类型的数据。用户验证并批准这些权限后，MCP客户端接收到安全调用MCP服务器所需的OAuth访问令牌。

安全性在这个简化过程中仍然是一个核心考虑因素，并带来重要的权衡。使合法服务器的连接变得更容易，不可避免地也使连接到恶意服务器变得更容易。该规范包括通过RFC 8707防止凭据盗用的技术保护措施，确保为一个服务器颁发的凭据不能被另一个服务器滥用。这意味着即使有人意外连接到欺骗性服务器（如Examp1e.com而不是Example.com），该服务器也无法访问用户在其他地方的合法账户。MCP客户端将特定服务器URL作为身份验证过程的一部分，授权服务器确保凭据严格限制在该服务器。

除了这些技术保护之外，MCP注册表工作组正在开发可信MCP服务器注册表的方法，类似于应用商店和软件包存储库如何帮助用户识别合法软件。虽然这项工作仍在进行中，但目标是帮助用户通过维护良好的已知MCP实现目录来发现和验证可信服务。

通过结合自动发现、动态注册和增强的安全控制，MCP已经自动化了传统上手动设置的过程，同时为这个更动态的世界添加了保护措施。
